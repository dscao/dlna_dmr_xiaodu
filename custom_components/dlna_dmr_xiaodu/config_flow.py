...
class DlnaDmrFlowHandler(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_ignore(
        ...
        for dev_type in DmrDevice.DEVICE_TYPES:
            discovery = await ssdp.async_get_discovery_info_by_udn_st(
                self.hass, self._location, dev_type
            )
            if discovery:
        ...
        return self.async_create_entry(
            ...
            data={
                ...
                CONF_DEVICE_ID: self._location.replace(":","_").replace("/","_"),
    ...
    def _create_entry(self) -> ConfigFlowResult:
        ...
        LOGGER.debug(
            "_async_create_entry: location: %s", self._location
        )
        assert self._location
    ...
    async def _async_set_info_from_discovery(
        ...
        LOGGER.debug(
            "_async_set_info_from_discovery: location: %s",
    ...
    async def _async_get_discoveries(self) -> list[SsdpServiceInfo]:
        ...
        discoveries = [
            disc for disc in discoveries if disc.ssdp_location not in current_unique_ids
        ]
...
def _is_ignored_device(discovery_info: ssdp.SsdpServiceInfo) -> bool:
    ...
    if manufacturer.startswith("lg") and "tv" in model:
        # webostv
        return True
    if manufacturer.startswith("DuerOS"):
        # DuerOS
        return True
